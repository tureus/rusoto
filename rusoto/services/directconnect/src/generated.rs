...
pub trait DirectConnect {
    ...
    #[doc="<p>Accept ownership of a private virtual interface created by another customer.</p> <p>After the virtual interface owner calls this function, the virtual interface will be created and attached to the given virtual private gateway or direct connect gateway, and will be available for handling traffic.</p>"]
    ...
    #[doc="<p>Creates an association between a direct connect gateway and a virtual private gateway (VGW). The VGW must be attached to a VPC and must not be associated with another direct connect gateway.</p>"]
    ...
    #[doc="<p>Deletes a direct connect gateway. You must first delete all virtual interfaces that are attached to the direct connect gateway and disassociate all virtual private gateways that are associated with the direct connect gateway.</p>"]
    ...
    #[doc="<p>Deletes the association between a direct connect gateway and a virtual private gateway.</p>"]
    ...
    #[doc="<p>Returns the list of AWS Direct Connect locations in the current AWS region. These are the locations that may be selected when calling <a>CreateConnection</a> or <a>CreateInterconnect</a>.</p>"]
    ...
    fn allocate_connection_on_interconnect(
        ...
        match response.status {
            StatusCode::Ok => {
                ...
                Ok(
                    serde_json::from_str::<Connection>(String::from_utf8_lossy(&body).as_ref())
                        .unwrap(),
                )
            }
            _ => {
    ...
    fn allocate_private_virtual_interface(
        ...
        match response.status {
            StatusCode::Ok => {
                let mut body: Vec<u8> = Vec::new();
                try!(response.body.read_to_end(&mut body));
                Ok(serde_json::from_str::<VirtualInterface>(
                    String::from_utf8_lossy(&body).as_ref(),
                ).unwrap())
            }
            _ => {
    ...
    fn confirm_connection(
        ...
        match response.status {
            StatusCode::Ok => {
                let mut body: Vec<u8> = Vec::new();
                try!(response.body.read_to_end(&mut body));
                Ok(serde_json::from_str::<ConfirmConnectionResponse>(
                    String::from_utf8_lossy(&body).as_ref(),
                ).unwrap())
            }
            _ => {
    ...
    fn create_bgp_peer(
        ...
        match response.status {
            StatusCode::Ok => {
                let mut body: Vec<u8> = Vec::new();
                try!(response.body.read_to_end(&mut body));
                Ok(serde_json::from_str::<CreateBGPPeerResponse>(
                    String::from_utf8_lossy(&body).as_ref(),
                ).unwrap())
            }
            _ => {
    ...
    fn create_interconnect(
        ...
        match response.status {
            StatusCode::Ok => {
                ...
                try!(response.body.read_to_end(&mut body));
                Ok(
                    serde_json::from_str::<Interconnect>(String::from_utf8_lossy(&body).as_ref())
                        .unwrap(),
                )
            }
            _ => {
    ...
    fn delete_bgp_peer(
        ...
        match response.status {
            StatusCode::Ok => {
                let mut body: Vec<u8> = Vec::new();
                try!(response.body.read_to_end(&mut body));
                Ok(serde_json::from_str::<DeleteBGPPeerResponse>(
                    String::from_utf8_lossy(&body).as_ref(),
                ).unwrap())
            }
            _ => {
    ...
    fn delete_interconnect(
        ...
        match response.status {
            StatusCode::Ok => {
                let mut body: Vec<u8> = Vec::new();
                try!(response.body.read_to_end(&mut body));
                Ok(serde_json::from_str::<DeleteInterconnectResponse>(
                    String::from_utf8_lossy(&body).as_ref(),
                ).unwrap())
            }
            _ => {
    ...
    fn delete_virtual_interface(
        ...
        match response.status {
            StatusCode::Ok => {
                let mut body: Vec<u8> = Vec::new();
                try!(response.body.read_to_end(&mut body));
                Ok(serde_json::from_str::<DeleteVirtualInterfaceResponse>(
                    String::from_utf8_lossy(&body).as_ref(),
                ).unwrap())
            }
            _ => {
    ...
    fn describe_connection_loa(
        ...
        match response.status {
            StatusCode::Ok => {
                let mut body: Vec<u8> = Vec::new();
                try!(response.body.read_to_end(&mut body));
                Ok(serde_json::from_str::<DescribeConnectionLoaResponse>(
                    String::from_utf8_lossy(&body).as_ref(),
                ).unwrap())
            }
            _ => {
    ...
    fn describe_connections(
        ...
        match response.status {
            StatusCode::Ok => {
                ...
                try!(response.body.read_to_end(&mut body));
                Ok(
                    serde_json::from_str::<Connections>(String::from_utf8_lossy(&body).as_ref())
                        .unwrap(),
                )
            }
            _ => {
    ...
    fn describe_interconnect_loa(
        ...
        match response.status {
            StatusCode::Ok => {
                let mut body: Vec<u8> = Vec::new();
                try!(response.body.read_to_end(&mut body));
                Ok(serde_json::from_str::<DescribeInterconnectLoaResponse>(
                    String::from_utf8_lossy(&body).as_ref(),
                ).unwrap())
            }
            _ => {
    ...
    fn describe_interconnects(
        ...
        match response.status {
            StatusCode::Ok => {
                ...
                try!(response.body.read_to_end(&mut body));
                Ok(
                    serde_json::from_str::<Interconnects>(String::from_utf8_lossy(&body).as_ref())
                        .unwrap(),
                )
            }
            _ => {
    ...
    fn describe_locations(&self) -> Result<Locations, DescribeLocationsError> {
        ...
        match response.status {
            StatusCode::Ok => {
                ...
                try!(response.body.read_to_end(&mut body));
                Ok(
                    serde_json::from_str::<Locations>(String::from_utf8_lossy(&body).as_ref())
                        .unwrap(),
                )
            }
            _ => {
    ...
    fn describe_virtual_interfaces(
        ...
        match response.status {
            StatusCode::Ok => {
                let mut body: Vec<u8> = Vec::new();
                try!(response.body.read_to_end(&mut body));
                Ok(serde_json::from_str::<VirtualInterfaces>(
                    String::from_utf8_lossy(&body).as_ref(),
                ).unwrap())
            }
            _ => {
